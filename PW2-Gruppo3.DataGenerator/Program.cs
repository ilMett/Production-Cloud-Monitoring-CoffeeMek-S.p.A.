...
namespace Pw2_Gruppo3.DataGenerator;
    ...
    public class Program
    {
        ...
        private static Message GenerateMessageData()
        {
            ...
            {
                Machine = faker.PickRandom(new[] { "Fresa CNC", "Tornio Verticale", "Centro di Lavoro Orizzontale", "Macchina a 5 Assi" }),
            ...
            var lathe = new Lathe()
            {
                Machine = faker.PickRandom(new[] { "Tornio automatico", "Tornio CNC", "Tornio Parallelo", "Tornio Verticale" }),
                MachineState = faker.PickRandom(new[] { "Operativa", "Inattiva", "Manutenzione", "Guasto" }),
                RotationSpeed = faker.Random.Int(1000, 3000).ToString(), // Esempio: 2094
                SpindleTemperature = faker.Random.Decimal(30.0m, 60.0m).ToString("0.00"), // Esempio: 35.71
                CompletedItems = faker.Random.Int(10, 100).ToString(), // Esempio: 57
                Site = faker.Address.Country(),
                TimeStampLocal = faker.Date.Recent(5).ToString("yyyy-MM-ddTHH:mm:ss"),
                TimeStampUtc = faker.Date.Recent(5).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ss"),
                MachineBlockage = isBlocked.ToString(), // Applica la stessa logica di blocco, convertendo in stringa
            ...
            {
                Machine = faker.PickRandom(new[] { "Linea Assemblaggio Robotizzata", "Linea Assemblaggio Semiautomatica", "Linea di Finitura", "Linea di Imballaggio" }),
                AverageStationTime = faker.Random.Decimal(15.0m, 45.0m).ToString("0.00"), // Esempio: 27.22
                OperatorsNumber = faker.Random.Int(1, 5).ToString(), // Esempio: 1
            ...
            {
                Machine = faker.PickRandom(new[] { "Linea di Test Funzionale", "Linea di Test di Stress", "Sistema di Ispezione Ottica", "Banco di Collaudo" }),
                TestResult = faker.PickRandom(new[] { "OK", "FAIL", "PASS" }), // Esempio: OK/FAIL
                BoilerPressure = faker.Random.Decimal(1.0m, 5.0m).ToString("0.00"), // Esempio: 3.39
                BoilerTemperature = faker.Random.Decimal(80.0m, 110.0m).ToString("0.00"), // Esempio: 95.47
                EnergyConsumption = faker.Random.Decimal(1.0m, 10.0m).ToString("0.00"), // Esempio: 3.26
                Site = faker.Address.Country(),
                TimeStampLocal = faker.Date.Recent(5).ToString("yyyy-MM-ddTHH:mm:ss"),
                TimeStampUtc = faker.Date.Recent(5).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ss"),
                MachineBlockage = isBlocked.ToString(), // Applica la stessa logica di blocco, convertendo in stringa
